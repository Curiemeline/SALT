...
class FinetuneWidget(QGroupBox):
    ...
    def save_masks(self):
        save_path = self.finetune_folder.text().strip()
        if not save_path:
    ...
    def split_dataset(self):
        finetune_dir = self.finetune_folder.text().strip()
        if not finetune_dir:
            show_info("Please specify a finetune folder.")
            return
        
        train_dir, test_dir = split_train_test(finetune_dir)
    ...
    def launch_finetuning(self):
        ...
        # label_layer_name = self.label_layer.currentText()
        # label_layer = self.viewer.layers.get(label_layer_name)
        ...
        # TODO: Call your real finetuning function here
        self.split_dataset()
        model_path, train_losses, test_losses = finetune_cellpose(output_path=folder, epochs=epochs, lr=learning_rate, model_name=model_name)
        save_loss_curve(f"{model_path}/models", train_losses, test_losses, model_name)
        show_info(f"Finetuning completed. Model saved to f{model_path}/models")
        print("Model saved to:", f"{model_path}/models")
